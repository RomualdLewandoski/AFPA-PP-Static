...
class Whence
{
    ...
    function push($uri='')
    {
        if($uri=='')
        {
            ...
            //echo "DEBUG: $uri";
/*            echo "DEBUG: $completeURL";
            if (strpos($completeURL, "webdev")){
                $uri = substr($completeURL, 25);
                if ($uri == ''){
                    $uri = "/";
                }

            }else {
                $uri = substr($completeURL, 18);
                if ($uri == ''){
                    $uri = "/";
                }
           ...
           /* $uri =strtolower(substr(stristr($_SERVER['REQUEST_URI'],'index.php?/'),11));
            if($uri=='')
            {
                $uri = strtolower(substr(stristr($_SERVER['REQUEST_URI'],''),0));
                if($uri=='')
                {
                    $uri = $this->homepage;
                }
